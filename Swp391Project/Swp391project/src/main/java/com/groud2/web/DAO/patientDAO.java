...
//        try {
//            String sql = "SELECT patient.doctorName,user.phonenumber, user.email,user.address,patient.timeOrder,patient.medicalDate,patient.medicalTime,patient.symptom,patient.conclude,patient.medicine,patient.doctorName,patient.status,patient.process,patient.Idcard\n"
//                    + "FROM patient INNER JOIN user  ON patient.userId=user.userId where doctorName=? and process=? and medicalDate=?";
//            connection = dbc.getConnection();
//            ps = connection.prepareStatement(sql);
...
//                    process = "Complete";
//                }
//               patient pa = new patient(patientName, Idcard, Idcard, timeOrder, medicalDate, medicalTime, symptom, conclude, medicine, doctorName, status, process);
...
//        ArrayList<patient> listwattingPatient = new ArrayList<>();
//        try {
//            String sql = "SELECT patient.doctorName, patient.timeOrder, patient.medicalDate, patient.medicalTime, patient.symptom, patient.conclude, patient.medicine, patient.status, patient.process, patient.Idcard\n"
//                    + "FROM patient WHERE patient.doctorName = ? AND patient.process = ? AND patient.medicalDate = ?";
//            connection = dbc.getConnection();
//            ps = connection.prepareStatement(sql);
//            ps.setString(1, fullname);
//            ps.setString(2, process);
